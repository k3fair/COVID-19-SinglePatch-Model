#     Script runs simulations for experiment on the impact of accounting 
...
#pick your scenario (Y/N on births+deaths, Y/N on NPIs)
birthdeath.select<-"no"
intervention.select<-"yes"

### GENERIC SETUP
years<- 10
samples<-500
popsample<- c(3e4, 5e4, 1e+05, 1e+06)
  ...
  for (z2 in 1:length(popsample))
  {
    ...
    if (intervention.select=="no") {parms[, c("tauI")]<-0; parms[, c("Tl")]<-0.99;}
    if (birthdeath.select=="no") {parms[,c("births", "deaths")]<-0;}
    
    inCstart <- parms[1,"Tl"]; # number of positive cases when province closed on Mar 17
    ...
    m3iter=function(S,parms,seed,Ns=parms[,"N"]){
      ...
      #Baseline infection probability. The last term with rowSums is how many people end up in each county for the day (not currently relevant as we have no travel between patches)
    ...
    df$infected<-rowSums(df[,colnames(df) %in% c("E",Di,Da)])
    df$know.current<-rowSums(df[,colnames(df) %in% Tk])
    df$currentpop<-rowSums(df[,colnames(df) %in% c("S", "E", Da, Di, "R")])
    
    df<- df %>% group_by(run) %>% mutate(new.K = Nt - lag(Nt), cum.D = cumsum(D))
    
    df$betaA0<-parms[1,"beta"]
    df$initpop<-pop
    
    df.init<-df[df$ts==min(df$ts),]
    outbreaks<-df.init$run[df.init$infected>0]
    ### Only keep runs that had an outbreak (matters for very small inital pops)
    df.outbreaks<-df[df$run %in% outbreaks,]

    df.outbreaks.2<-df.outbreaks[,colnames(df.outbreaks) %in% c("ts","infected", "initpop", "run")]
    
    if (z2==1) {
      df.ind<-df.outbreaks.2
    } else {
      df.ind<-rbind(df.ind, df.outbreaks.2)
    }
...
if (intervention.select=="yes") {str1<-"npi"} else {str1<-"NOnpi"}
if (birthdeath.select=="yes") {str2<-"birthdeath"} else {str2<-"NObirthdeath"}

write.csv(df.ind, sprintf("COVIDsim_birthdeathexp_%s_%s.csv", str1, str2), row.names = FALSE)
